...
describe("Test payload validation", () => {
  ...
  it("Field in the payload and required", () => {
    const validator = new Validator(payload, [{ required: "required" }]);
    expect(validator.validate()).to.equal(true);
    expect(validator.errors()).to.equal(null);
  });
  it("Field in the payload is a json", () => {
    const validator = new Validator(payload, [{ json: "required|json" }]);
    expect(validator.validate()).to.equal(true);
    expect(validator.errors()).to.equal(null);
  });
  it("Field in the payload is an array", () => {
    const validator = new Validator(payload, [{ array: "required|array" }]);
    expect(validator.validate()).to.equal(true);
    expect(validator.errors()).to.equal(null);
  });
  ...
  it("Field in the payload is not in an email", () => {
    const validator = new Validator(payload, [{ email: "required|email" }]);
    expect(validator.validate()).to.equal(true);
    expect(validator.errors()).to.equal(null);
  });
  ...
  it("Field in the payload is an url", () => {
    const validator = new Validator(payload, [{ url: "required|url" }]);
    expect(validator.validate()).to.equal(true);
    expect(validator.errors()).to.equal(null);
  });
  it("Field in the payload is in an ip (ipv4)", () => {
    const validator = new Validator(payload, [{ ip: "required|ip" }]);
    expect(validator.validate()).to.equal(true);
    expect(validator.errors()).to.equal(null);
  });
  it("Field in the payload is not an ipv6", () => {
    const validator = new Validator(payload, [{ ipv6: "required|ipv6" }]);
    expect(validator.validate()).to.equal(true);
    expect(validator.errors()).to.equal(null);
  });
  it("Field in the payload is boolean", () => {
    const validator = new Validator(payload, [{ boolean: "required|boolean" }]);
    expect(validator.validate()).to.equal(true);
    expect(validator.errors()).to.equal(null);
  });
  ...
  it("Field in the payload is date", () => {
    const validator = new Validator(payload, [{ date: "required|date" }]);
    expect(validator.validate()).to.equal(true);
    expect(validator.errors()).to.equal(null);
  });
